...
{
  // tambahan
  long pix = Camera::HEIGHT * Camera::WIDTH;
  accumulator = new int[pix];
...
vector<Point2D> ColorFinder::getBlobCenter(Image *img, vector<Point2D> fieldBorderPoints)
{
  ...
  // Calculate center
  /* Pencarian luas blob terbesar yang kemudian akan menjadi titik acuan terhadap bola oleh robot humanoid
  */
  int sizeball(0);
	...
	for (int i = 0; i < Camera::HEIGHT; i++)
  {
    for(int j = 0; j < Camera::WIDTH; j++)
    {
               ...
               img->m_ImageData[(i * Camera::WIDTH + j) * img->m_PixelSize + 2] == 0)
		    {
		    	...
		    	if(
		    		(img->m_ImageData[((i-1) * Camera::WIDTH + j) * img->m_PixelSize + 0] != 255 &&
         	   		img->m_ImageData[((i-1) * Camera::WIDTH + j) * img->m_PixelSize + 1] != 128 &&
               		img->m_ImageData[((i-1) * Camera::WIDTH + j) * img->m_PixelSize + 2] != 0)
               		|| (img->m_ImageData[(i * Camera::WIDTH + j-1) * img->m_PixelSize + 0] != 255 &&
         	   			img->m_ImageData[(i * Camera::WIDTH + j-1) * img->m_PixelSize + 1] != 128 &&
               			img->m_ImageData[(i * Camera::WIDTH + j-1) * img->m_PixelSize + 2] != 0)
               		|| (img->m_ImageData[(i * Camera::WIDTH + j+1) * img->m_PixelSize + 0] != 255 &&
         	   			img->m_ImageData[(i * Camera::WIDTH + j+1) * img->m_PixelSize + 1] != 128 &&
               			img->m_ImageData[(i * Camera::WIDTH + j+1) * img->m_PixelSize + 2] != 0)
               		|| (img->m_ImageData[((i+1) * Camera::WIDTH + j) * img->m_PixelSize + 0] != 255 &&
...
void ColorFinder::Detect(Image * img, int r_min, int r_max)
{
  int x, y;
  bool find = false;
  ...
  for (int r = r_max; r >= r_min; r--)
  {
    ...
    while (i < img->m_NumberOfPixels && !find) {
      if (accumulator[i] > thresh) {
        ...
        //Draw::Cross(img, Point2D(x,y), 5, ColorRGB(0,0,255));
        drawCircle(img, x, y, r);
        Draw::Circle(img, Point2D(x,y), 3, ColorRGB(255,0,0));
        find = true;
      }
      i++;
    }
...
void ColorFinder::drawPixel(Image * img, int x, int y) {
  if (x < Camera::WIDTH && x >= 0
    && y < Camera::HEIGHT && y >= 0)
    {
      img->m_ImageData[(Camera::WIDTH*(y) + x) * img->m_PixelSize + 0] = 255;
      img->m_ImageData[(Camera::WIDTH*(y) + x) * img->m_PixelSize + 1] = 255;
      img->m_ImageData[(Camera::WIDTH*(y) + x) * img->m_PixelSize + 2] = 0;
    }
}
